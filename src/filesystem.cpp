...
static RecursiveMutex *s_lfs_mutex = 0;
...
int filesystem_lock(filesystem_t* fs) {
    (void)fs;
    if (!s_lfs_mutex) {
        s_lfs_mutex = new RecursiveMutex();
    }
    if (s_lfs_mutex) {
        s_lfs_mutex->lock();
    }
            ...
            lfs_off_t off, void* buffer, lfs_size_t size)
{
    ...
    LittleFS::getFlashInstance()->readData((LittleFS::getInstance()->getStartBlock() + block) * c->block_size + off, (uint8_t*)buffer, size);
    return 0;
}
            ...
            lfs_off_t off, const void* buffer, lfs_size_t size)
{
    ...
    LittleFS::getFlashInstance()->writeData((LittleFS::getInstance()->getStartBlock() + block) * c->block_size + off, buffer, size);
    return 0;
}
...
int fs_erase(const struct lfs_config* c, lfs_block_t block)
{
    ...
    LittleFS::getFlashInstance()->sectorErase((LittleFS::getInstance()->getStartBlock() + block) * c->block_size);
    return 0;
}
...
int filesystem_mount(filesystem_t* fs) {
    ...
    for(int tries = 0; tries < 3; tries++) {
        ...
        if (!ret) {
            ret = lfs_mkdir(&fs->instance, "/usr");
            if (ret == 0 || ret == LFS_ERR_EXIST) {
                fs->state = true;
                return 0;
            }
            // Failed to mkdir, probably LFS_CORRUPTED (-52), loop around and
            // try formatting
            Log.info("lfs_mkdir=%d tries=%d", ret, tries);
        }
